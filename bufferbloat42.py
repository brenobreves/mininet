...
def start_iperf(net):
    ...
    h4.popen(f"iperf -c {h1.IP()} -p 5002 -t {args.time}")

    h3.popen(f"iperf -c {h1.IP()} -p 5003 -t {args.time}")
...
def start_ping(net):
    ...
    pings.append(
        h2.popen(
            f'ping {h1.IP()} -i 0.1 > {args.dir}/ping42_h2_reno.txt',
            shell=True
        )
    )
    ...
    pings.append(
        h4.popen(
            f'ping {h1.IP()} -i 0.1 > {args.dir}/ping42_h4_reno.txt',
            shell=True
        )
    )
    ...
    pings.append(
        h3.popen(
            f'ping {h1.IP()} -i 0.1 > {args.dir}/ping42_h3_bbr.txt',
            shell=True
        )
    )
    ...
    pings.append(
        h5.popen(
            f'ping {h1.IP()} -i 0.1 > {args.dir}/ping42_h5_bbr.txt',
            shell=True
        )
    )
